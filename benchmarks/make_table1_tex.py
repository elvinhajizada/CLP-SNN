...
def emit_primitives(led):
    ...
    L.append(r"         the mean over all stream samples; the 99th percentile is given")
    L.append(r"         per primitive and is not additive across primitives. The")
    L.append(r"         explicit-inversion SLDA rows are a reference implementation")
    L.append(r"         that inverts the $1280 \times 1280$ regularized covariance at")
    L.append(r"         every sample; they are reported to quantify the gain of the")
    L.append(r"         rank-one update, not as the paper's SLDA baseline.")
    L.append(r"         Substituting the rank-one update removes 21.4~ms of the")
    L.append(r"         22.9~ms per-sample update on the GPU (94\%), consistent with")
    L.append(r"         the arithmetic: one LU inversion costs $\approx \tfrac{2}{3}d^3")
    L.append(r"         \approx 1.4 \times 10^{9}$ operations against $\approx 3.4")
    L.append(r"         \times 10^{6}$ for the rank-one update, a factor of about 400.")
    L.append(r"         In the FP16 rows the statistics are maintained in FP32 and")
    L.append(r"         half precision is applied to the matrix products only")
    L.append(r"         (Methods).}")
